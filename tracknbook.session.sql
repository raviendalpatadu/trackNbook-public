...
FROM tbl_inquiry i
    JOIN tbl_user u ON i.inquiry_passenger_id = u.user_id
    JOIN tbl_staff_ticketing st ON i.inquiry_station = st.staff_ticketing_station
    JOIN tbl_reservation r ON i.inquiry_ticket_id = r.reservation_ticket_id
...
WHERE i.inquiry_id = 1    AND i.inquiry_station = st.staff_ticketing_station;